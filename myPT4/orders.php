...
<head>
  ...
  <title>Apple Products Ordering System : Orders</title>
  <!-- Bootstrap -->
...
<body style="background-image: url('applebg.jpg');">
  ...
  <div class="row">
    <div class="col-xs-12 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
      ...
      <div class="form-group">
          ...
          <div class="col-sm-9">
          <input name="oid" type="text" class="form-control" id="oid" placeholder="Order ID" value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_num']; ?>" readonly>
        </div>
        </div>
        <div class="form-group">
  ...
  <div class="form-group">
      ...
      <div class="col-sm-9">  
      <select name="cid" class="form-control" id="customers" required>
      <option value=""selected disabled>Please Select</option>
      <?php
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_customers_a176664_pt2");
        $stmt->execute();
        $result = $stmt->fetchAll();
      }
      catch(PDOException $e){
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a176664, tbl_staffs_a176664_pt2, tbl_customers_a176664_pt2 WHERE ";
        $sql = $sql."tbl_orders_a176664.fld_staff_num = tbl_staffs_a176664_pt2.fld_staff_id and ";
        $sql = $sql."tbl_orders_a176664.fld_customer_num = tbl_customers_a176664_pt2.fld_customer_id LIMIT $start_from, $per_page";
      ...
      <tr>
        ...
        <td>
          <a href="orders_details.php?oid=<?php echo $orderrow['fld_order_num']; ?>"class="btn btn-info btn-xs" role="button">Details</a>
           <a href="<?php if ($userlevel=="Admin"){ echo "orders.php?edit="; echo $orderrow['fld_order_num'];} ?>"class="btn btn-success btn-xs" role="button" onclick="<?php if ($userlevel=="Staff") echo "return confirm('Sorry, Only Admin Can Access This !');"?>" <?php if ($userlevel=="Staff") echo "disabled"?>> Edit</a>
           <a href="<?php if ($userlevel=="Admin"){ echo "orders.php?delete="; echo $orderrow['fld_order_num'];} ?>"class="btn btn-danger btn-xs" role="button" onclick="<?php if ($userlevel=="Staff") echo "return confirm('Sorry, Only Admin Can Access This !');"?>" <?php if ($userlevel=="Staff") echo "disabled"?>> Delete</a>
        </td>
      </tr>
      <?php
      }
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a176664");
            $stmt->execute();
            $result = $stmt->fetchAll();
